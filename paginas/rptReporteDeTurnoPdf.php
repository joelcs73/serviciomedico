...
class PDF extends FPDF {
    ...
    function Header(){
        ...
        $this->Cell(40,5,utf8_decode('Reporte de turno.'),0,1,'L');
        $this->SetY(20);
   ...
   // Colores, ancho de línea y fuente en negrita
    ...
    // Cabecera
        $header = array("HORA","NOMBRE","SEXO","EDAD","DEPTO","ATENDIÓ","DATOS","DX","RP");
        $w = array(10, 50, 13, 10, 55, 40 ,30, 45, 90);
    ...
    // Restauración de colores y fuentes
        ...
        while($row = mysqli_fetch_assoc($result)){
            $this->Cell($w[0],4,utf8_decode($row["hora"]),$marcoDeCelta,0,'C',$fill);
            $this->Cell($w[1],4,utf8_decode($row["nombre"]),$marcoDeCelta,0,'L',$fill);
            $this->Cell($w[2],4,utf8_decode($row["sexo"]),$marcoDeCelta,0,'L',$fill);
            $this->Cell($w[3],4,utf8_decode($row["edad"]),$marcoDeCelta,0,'C',$fill);
            
            $this->SetFont('Arial','',5);
            // $this->Cell($w[4],4,utf8_decode($row["nombreDepartamento"]),$marcoDeCelta,0,'L',$fill);
            $y=$this->GetY();
            $this->Multicell($w[4],4,utf8_decode($row["nombreDepartamento"]),$marcoDeCelta,'L',$fill);

            $this->SetFont('Arial','',6);
            $this->SetY($y);
            $this->SetX(143);
            $this->Cell($w[5],4,utf8_decode($row["atendidoPor"]),$marcoDeCelta,0,'L',$fill);
            $this->Cell($w[6],4,utf8_decode($row["datos"]),$marcoDeCelta,0,'L',$fill);
            
            $this->Cell($w[7],4,utf8_decode($row["dx"]),$marcoDeCelta,0,'L',$fill);



            $this->SetFont('Arial','',5);
            // $this->Cell($w[7],4,utf8_decode($row["rp"]),$marcoDeCelta,0,'L',$fill);
            $y2=$this->GetY();
            $this->Multicell($w[8],4,utf8_decode($row["rp"]),$marcoDeCelta,'L',$fill);
...
$fecha = $_GET["f"];
$fechaDMA = voltear_fecha_dma($_GET["f"]);
...
$strQry = $objPaciente->qryReporteDeTurno($fecha);
